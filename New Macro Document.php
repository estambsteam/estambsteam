<?php
/* Admin And Token */
$token = 312471242:AAEggR1BJj_GorgNQn6-KSdFy6ThKe__bTw;
$admin = 183314524;
$admin_username = @ellijooonn;
$admin_number = 17193236931;
$admin_name = гАДгр лФФД;
/*================*/
...
elseif(preg_match('/^\/([Ss]tats)/',$text) and $from_id == $admin){
    ...
    norbert('sendMessage',[
      'chat_id'=>$chat_id,
      'text'=>"йзого ≤А гзжг: $member_count",
      'parse_mode'=>'HTML'
    ]);
}
elseif(preg_match('/^\/([Cc]ontact)/',$text) and $from_id == $admin){
    ...
    norbert('sendMessage',[
      'chat_id'=>$chat_id,
      'text'=>"йзого тЦгяЕ ЕгМ сМФ тоЕ: $tel_count",
      'parse_mode'=>'HTML'
    ]);
}
elseif($contact!=null and $from_id != $admin){
	...
	norbert('sendMessage',[
      'chat_id'=>$chat_id,
      'text'=>"тЦгяй сМФ то йФ ЕЦ сМФ ≤Д хМг │М ФМ
	  $admin_username",
      'parse_mode'=>'HTML',
	  'reply_to_message_id'=>$message_id
    ]);
